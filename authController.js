...
import {EMAIL_VERIFY_TEMPLATE, PASSWORD_RESET_TEMPLATE} from '../config/emailTemplate.jsx';
...
export const register = async (req, res) => {
  ...
  try {
    ...
    res.cookie('token', token, {
      ...
      // Sending Welcome Options
      const mailOptions = {
        from : process.env.SENDER_EMAIL,
        to: email,
        Subject: 'Welcome to GreatStack',
        text :  `Welcome to greatSteack website. Your account has been created with email id ${email}`
      }
...
export const sendResetOTp = async (req, res) => {
 ...
 const otp =  String(Math.floor(100000 + Math.random() * 900000));
    ...
    const mailOption = {
       ...
       // text :  `Your OTP for resetting your password is ${otp} Use this OTP to proceed with resetting for password`,
...
export const resetPassword = async (req, res) => {
  ...
  if(!email  || !otp ||!newPassword ){
    return res.json({success: false, message: 'Email, OTP and newPassword are rquired'})
  }
 try{
  const user = await userModel.findOne({email});
  if(!user){
